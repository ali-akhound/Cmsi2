...
function SubmitForm(url, form, data, $http, toaster, blockUIService, successCallback, errorCallback) {
    if (validateForm(form)) {
        blockUIService.start();
        if ($(form).find('input[type="file"]').length > 0) {
            ajaxFileFormPost($http, url, data, toaster, blockUIService, successCallback)
        }
        else {
            ajaxPost($http, url, data, toaster, blockUIService, successCallback, errorCallback)
        }
    }
    /*debugger;
...
function ajaxPost($http, url, data, toaster, blockUIService, successCallback, errorCallback, forgeryTokenID) {
    ...
    if (errorCallback == undefined || errorCallback == null) {
        errorCallback = function errorCallback(xhr, err) {
            ...
            if (xhr.status == 403) {
                ...
                ShowToast(toaster, 'Unauthorized! You have not enough permission to fulfill this action.', 'error');
            } else if (xhr.status == 404) {
                ShowToast(toaster, 'Your requested item was not found.', 'error');
            }
            else {
                if (xhr.data.error != undefined)
                    ShowToast(toaster, xhr.data.error, "error");
                else
                    ShowToast(toaster, xhr.statusText, "error");
            }
        }
    }
    $http({
...
function ajaxFileFormPost($http, url, data, toaster, blockUIService, successCallback, errorCallback, forgeryTokenID) {
    if (forgeryTokenID == null || forgeryTokenID == undefined) {
        forgeryTokenID = 'forgeryToken';
    }
    if (errorCallback == undefined || errorCallback == null) {
...
function handleDeleteAction($http, deleteUrl, ids, Grid) {
    swal({
        ...
        ajaxPost($http, deleteUrl, ids, null, null, function successCallback(response) {
...
function handleAction($http, ActionUrl, dataJson, toaster, blockUIService, successCallback, forgeryTokenID) {
    ajaxPost($http, ActionUrl, dataJson, toaster, blockUIService, successCallback, null, forgeryTokenID);
...
function MySubmitController($scope, $http, blockUI, toaster, $templateCache, $state, $templateCache) {
    ...
    $scope.submitForm = function (event, ActionUrl, FormID) {
        ...
        var thisForm = $('#' + FormID);
        SubmitForm(ActionUrl, thisForm, $scope.model, $http, toaster, blockUI, function successCallback(response) {
            blockUI.stop();
    ...
    /***************************MailTemplate***************************/
    $scope.updateTemplate = function (sender) {
        var selectedVal = $scope.model.SelectedItem;
        for (var i = 0; i < $scope.model.TotalMailTemplate.length; i++) {
            if ($scope.model.TotalMailTemplate[i].ID == selectedVal) {
                $scope.model.SingleMailTemplate = $scope.model.TotalMailTemplate[i];
                $scope.model.SelectedItem = $scope.model.DprItems[i];
                HtmlEditor.SetHtml($scope.model.TotalMailTemplate[i].Template);
                CurrentModel.CurrentID = i;
                return;
            }
        }
    ...
    /***************************UserRole***************************/
    $scope.searchRole = function ($select) {
        return $http.post(feedUrl, {
            Input: { 'Name': $select.search, 'Roles': $scope.model.Roles }
        }).then(function (response) {
            $scope.model.RolesFeed = response.data;
        });
    }
    /***************************UserRole***************************/
    ...
    $scope.ForgetPass = function (event, ActionUrl, FormID) {
        event.preventDefault();
        var thisForm = $('#' + FormID);
        SubmitForm(ActionUrl, thisForm, $scope.model, $http, toaster, blockUI, function successCallback(response) {
            blockUI.stop();
            ...
            if (response.data.Type === "Error") {
                ShowSweetAlert(response.data.error.join(','), 'error')
            }
        },
            function errorCallback(response) {
                blockUI.stop();
                ShowSweetAlert(response.data.error, 'error')
            })
    }
    ...
    $scope.searchCategory = function ($select) {
        return $http.post(feedUrl, {
            Input: { 'Name': $select.search, 'Categories': $scope.model.Categories }
        }).then(function (response) {
            $scope.model.CategoriesFeed = response.data;
        });
    }

    /*************************** Csmi ***************************/
    $scope.searchReferee = function ($select) {
        if ($state.current.name == 'ArticleManagement') {
            return $http.post(feedUrl, {
                Input: { 'Name': $select.search, 'Referees': $scope.model.SendArticleViewModel.Referees, 'ConferenceID': $scope.model.SendArticleViewModel.ConferenceID }
            }).then(function (response) {
                $scope.model.SendArticleViewModel.RefereesFeed = response.data;
            });
        }
        if ($state.current.name == 'ConferenceReferee') {
            return $http.post(feedUrl, {
                Input: { 'Name': $select.search, 'Referees': $scope.model.Referees, 'ConferenceID': $scope.model.ConferenceID }
            }).then(function (response) {
                $scope.model.RefereesFeed = response.data;
            });
        }
    }
    $scope.searchExecutor = function ($select) {
        return $http.post(feedUrl, {
            Input: { 'Name': $select.search, 'Executors': $scope.model.Executors, 'ConferenceID': $scope.model.ConferenceID }
        }).then(function (response) {
            $scope.model.ExecutorsFeed = response.data;
        });
    }
    $scope.searchScientificSecretary = function ($select) {
        return $http.post(feedUrl, {
            Input: { 'Name': $select.search, 'ScientificSecretaries': $scope.model.ScientificSecretaries, 'ConferenceID': $scope.model.ConferenceID }
        }).then(function (response) {
            $scope.model.ScientificSecretariesFeed = response.data;
        });
    }

    $scope.searchDefinedCategories = function ($select) {
        return $http.post(feedUrl, {
            Input: { 'Name': $select.search, 'DefinedCategories': $scope.model.DefinedCategories, 'ConferenceID': $scope.model.ConferenceID }
        }).then(function (response) {
            $scope.model.DefinedCategoriesFeed = response.data;
        });
    }
    /*RefereeQuestion*/
    $scope.DeleteAnswer = function (index) {
        if (confirm('آیا از حذف اطمینان دارید؟'))
            $scope.model.RefereeAnswers.splice(index, 1);
    }
    $scope.DeleteServerAnswer = function (ID, deleteUrl) {
        var ids = [];
        ids.push(ID);
        if (confirm('آیا از حذف اطمینان دارید؟')) {
            ajaxPost($http, deleteUrl, ids, null, null, function successCallback(response) {
                var index = $scope.model.RefereeAnswers.map(function (el) {
                    return el.ID;
                }).indexOf(ID)
                $scope.model.RefereeAnswers.splice(index, 1);
                swal("Deleted!", "با موفقیت حذف شد", "success");
    ...
    $scope.submitRefereeAnswer = function (event, formIsValid) {
        event.preventDefault();
        $scope.submitted = true;
        if (formIsValid) {
            if ($scope.model.RefereeAnswers == null) {
                $scope.model.RefereeAnswers = [];
            }
            var copiedObject = jQuery.extend({}, $scope.model.RefereeAnswer);
            copiedObject.ID = $scope.model.RefereeAnswers.length;
            $scope.model.RefereeAnswers.push(copiedObject);
        }
        else {
            return formIsValid;
        }
    ...
    $scope.submitRefereeServerAnswer = function (event, formIsValid, ActionUrl, FormID) {
        event.preventDefault();
        $scope.submitted = true;
        if (formIsValid) {
            var thisForm = $('#' + FormID);
            var copiedObject = jQuery.extend({}, $scope.model.RefereeAnswer);
            copiedObject.QuestionID = $scope.model.ID;
            copiedObject.ObjectState = 1;
            SubmitForm(ActionUrl, thisForm, copiedObject, $http, toaster, blockUI, function successCallback(response) {
                blockUI.stop();
                if (response.data.Type == "Success") {
                    $scope.model.RefereeAnswers.push(response.data.Data);
                    ShowToast(toaster, 'با موفقیت ثبت شد', 'success');
                }
                else {

                    ShowToast(toaster, 'خطا در ثبت', 'error');
                }
            })
        }
        else {
    ...
    $scope.EditAnswer = function (event) {
        event.preventDefault();
        $scope.model.ActiveEditMode = false;
        $scope.model.RefereeAnswer = null;
    }
    $scope.EditServerAnswer = function (event, ActionUrl, FormID) {
        event.preventDefault();
        var thisForm = $('#' + FormID);
        var copiedObject = jQuery.extend({}, $scope.model.RefereeAnswer);
        copiedObject.QuestionID = $scope.model.ID;
        copiedObject.ObjectState = 2;
        SubmitForm(ActionUrl, thisForm, copiedObject, $http, toaster, blockUI, function successCallback(response) {
            blockUI.stop();
            if (response.data.Type == "Success") {
                ShowToast(toaster, 'با موفقیت ثبت شد', 'success');
    ...
    $scope.ActiveAnswerEditMode = function (index) {
        $scope.model.ActiveEditMode = true;
        $scope.model.RefereeAnswer = $scope.model.RefereeAnswers[index];
    }
    $scope.ActiveAnswerServerEditMode = function (ID) {
        $scope.model.ActiveEditMode = true;
        $scope.CurrentID = ID;
        var index = $scope.model.RefereeAnswers.map(function (el) {
            return el.ID;
    ...
    $scope.SendArticle = function (event, ActionUrl, FormID) {
        ...
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $scope.model.SendArticleViewModel.selectedArticleIDs = selectedIDs.split(',');
                var thisForm = $('#' + FormID);
                SubmitForm(ActionUrl, thisForm, $scope.model.SendArticleViewModel, $http, toaster, blockUI, function successCallback(response) {
                    blockUI.stop();
                    if (response.data.Type == "Success") {
    ...
    $scope.SetArticlePresentType = function (event, ActionUrl, FormID) {
        event.preventDefault();
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $scope.model.ArticleStatusTypeViewModel.selectedArticleIDs = selectedIDs.split(',');
                var thisForm = $('#' + FormID);
                SubmitForm(ActionUrl, thisForm, $scope.model.ArticleStatusTypeViewModel, $http, toaster, blockUI, function successCallback(response) {
                    blockUI.stop();
                    if (response.data.Type == "Success") {
                        ArticleManagementGrid.PerformCallback();
                        $('#myModal3').modal('hide');
    ...
    $scope.SetArticleStatusType = function (event, ActionUrl, FormID) {
        event.preventDefault();
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $scope.model.StatusTypeViewModel.selectedArticleIDs = selectedIDs.split(',');
                var thisForm = $('#' + FormID);
                SubmitForm(ActionUrl, thisForm, $scope.model.StatusTypeViewModel, $http, toaster, blockUI, function successCallback(response) {
                    blockUI.stop();
                    if (response.data.Type == "Success") {
                        ArticleManagementGrid.PerformCallback();
                        $('#myModal3').modal('hide');
    ...
    /*ArticleList*/
    $scope.ArticleStatusType = false;
    $scope.ArticlePosterEnabled = false;
    $scope.EnableExplain = true;
    $scope.ChangeArticleStatus = function () {
        if ($scope.model.ArticleStatusTypeViewModel.SelectedStatusID == 4 || $scope.model.ArticleStatusTypeViewModel.SelectedStatusID == 6) {
            $scope.EnableExplain = false;
        }
        else {
            $scope.EnableExplain = true;
        }
        if ($scope.model.ArticleStatusTypeViewModel.SelectedStatusID == 5) {
            $scope.ArticleStatusType = false;
            if ($scope.model.ArticleStatusTypeViewModel.SelectedPresentTypeID == 3) {
                $scope.ArticlePosterEnabled = true;
            }
        }
        else {
            $scope.model.ArticleStatusTypeViewModel.SelectedPresentTypeID = jQuery.grep($scope.model.ArticleStatusTypeViewModel.ArticlePresentTypes, function (a) {
                return a.Value == '1';
            })[0].Value;
            $scope.ArticleStatusType = true;
        }
    }
    $scope.ChangeArticlePresentType = function () {
        if ($scope.model.ArticleStatusTypeViewModel.SelectedPresentTypeID == 3) {
            $scope.ArticlePosterEnabled = true;
        }
        else {
    ...
    $scope.VoteCheckChange = function (candidate) {
        ...
        if ($scope.model.candidCnt >= 6) {
            ...
            $scope.model.candidCnt--;
            ShowToast(toaster, 'تعداد آرای اعضا شما 5 عدد می باشد', 'error')
        }
        if ($scope.model.inspectorCnt >= 2) {
            candidate.IsChecked = false;
            $scope.model.inspectorCnt--;
            ShowToast(toaster, 'تعداد آرای بازرس شما 1 عدد می باشد', 'error')
        }
    }
    $scope.SendActivationCode = function (event,ActionUrl) {
        event.preventDefault();
        blockUI.start();
        handleAction($http, ActionUrl, null, toaster, blockUI, function successCallback(response) {
            blockUI.stop();
            ShowToast(toaster, response.data.Data, 'success')
        }, 'forgeryToken');
    }
}
function MyGridController($scope, $http, $state, blockUI, toaster, blockUI, $templateCache, $uibModal) {
    ...
    $scope.EditAction = function (stateName) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(stateName, { id: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.ActiveAction = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
                handleAction($http, ActionUrl, data, toaster, blockUI, function successCallback(response) {
                    ShowToast(toaster, response.data.Data, 'success');
                    eval(Grid).Refresh();
                }, 'forgeryToken');
            }
    ...
    /****************************UserManagement***************************/
    $scope.AssignRole = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
    ...
    /***************************Category***************************/
    $scope.GotoCategory = function (TableNameInput) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go('CategoryManagement', { TableName: TableNameInput, ParentID: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.AddFeature = function () {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go('FeaturesManagement', { CatID: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.AddFeatureWithCat = function (CatID) {
        $state.go('FeaturesManagement', { CatID: CatID });
    }
    $scope.EditFeature = function (state, CatID) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { id: selectedIDs.split(',')[0], CatID: CatID });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.AssignModule = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { roleID: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    /*************************** Csmi ***************************/
    /*Conference*/
    $scope.AssignReferee = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { ConferenceID: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.AssignExecutor = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { ConferenceID: selectedIDs.split(',')[0] });
            }
    ...
    $scope.AssignScientificSecretary = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { ConferenceID: selectedIDs.split(',')[0] });
            }
    ...
    $scope.AssignCategory = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { ConferenceID: selectedIDs.split(',')[0] });
            }
    ...
    $scope.ConferenceCompanion = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { ConferenceID: selectedIDs.split(',')[0] });
            }
    ...
    $scope.ConferenceArticle = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { ConferenceID: selectedIDs.split(',')[0] });
            }
    ...
    $scope.GotoConferenceReferee = function () {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go('ConferenceRefereeManagement', { Conference: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.GotoConferenceExecutor = function () {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go('ConferenceExecutorManagement', { Conference: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.OpenSendArticleModal = function () {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $('#myModal2').modal('show');
                $('#myModal2 .select2-search-field input').width(300);
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.OpenPeresentArticleModal = function () {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $("#SendArticleForm input,textarea").each(function (index) {
                    $(this).val('');
                });
                $('#myModal3').modal('show');
            }
    ...
    $scope.OpenStatusArticleModal = function () {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $('#myModal4').modal('show');
            }
    ...
    /*SocietyMember*/
    $scope.AssignSocietyMember = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { PeriodID: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    /*GalleryImage*/
    $scope.AssignImage = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { AlbumID: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    /*Payment*/
    $scope.ConfirmPayment = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.NotConfirmPayment = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.GetArchivedArticles = function (Url) {
        ajaxPost($http, Url, null, null, null, function successCallback(response) {
    ...
    /*Election*/
    $scope.ElectionCandidate = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { ElectionID: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
    ...
    $scope.GoElectionSubState = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { id: selectedIDs.split(',')[0] });
            }
    ...
    /*
    $scope.ActiveReferee = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.ActiveConference = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.ActiveArticle = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.ActiveConferenceReferee = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.ActiveUser = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.ActiveNews = function (ActionUrl, Grid) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                var data = new Object();
                data.ids = selectedIDs.split(',');
    ...
    $scope.AssignArticle = function (state) {
        if (selectedIDs != undefined) {
            if (selectedIDs.split(',').length > 0) {
                $state.go(state, { Conference: selectedIDs.split(',')[0] });
            }
            else {
                ShowToast(toaster, 'لطفا یک مورد را انتخاب نمایید', 'error')
            }
...
function MyLayoutController($scope, $http, blockUI, toaster, $templateCache, $state) {
    $scope.SignOut = function (ActionUrl, logOutUrl) {
        handleAction($http, ActionUrl, [], toaster, blockUI, function successCallback(response) {
            if (response.data.Data == '1') {
...
angular
    ...
    */
    ...
    .controller('translateCtrl', ['$translate', '$scope', translateCtrl])
    .controller('MySubmitController', ['$scope', '$http', 'blockUI', 'toaster', '$templateCache', '$state', '$templateCache', MySubmitController])
    .controller('MyGridController', ['$scope', '$http', '$state', 'blockUI', 'toaster', 'blockUI', '$templateCache', MyGridController])
    .controller('MyLayoutController', ['$scope', '$http', 'blockUI', 'toaster', '$templateCache', '$state', MyLayoutController])
