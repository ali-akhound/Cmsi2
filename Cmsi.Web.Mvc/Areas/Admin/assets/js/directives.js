...
function Directive() {
    return {
        template: '<select style="margin-left:0px;width:100%" ng-model="directiveModel" ng_options = "s.id as s.text for s in directiveModel track by s.id"></select>',
        replace: true,
        restrict: 'E',
        /*require: 'ngModel',/*/
        scope: {
        ...
        link: function (_scope, _element) {
            var selectOptions = {
                ...
                /*/
                ajax: {
                    url: _scope.url + (_scope.urlQueryString != '' && _scope.urlQueryString != undefined ? ('?' + _scope.urlQueryString) : ''),
                    quietMillis: 250,
                    dataType: 'json',
                    method: 'POST',
                    data: function (term) {
                        var d = new Object();
                        d[_scope.queryParam] = term;
                        return d;
                    },
            ...
            $(_element).addClass(_scope.containerCssClass);
            ...
            if (_scope.selectedItem) {
                ...
                $(_element).empty();
                $.each(_scope.selectedItem, function (index, key) {
                    currentSelectedValues.push(key.id);
                    $(_element).append('<option value="' + key.id + '">' + key.text + '</option>');
                });
                setTimeout(function () {
                    /* $(_element).val(currentSelectedValues).trigger('change'); /*/
                }, 1000);
            }
        }
    };
...
angular
    ...
    .directive("filesInput", fixFileInput);
